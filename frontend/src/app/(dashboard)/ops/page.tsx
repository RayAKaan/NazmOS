        ...
        <Panel title="Recent uploads" subtitle="Fix bad imports before the merchant call.">
          <div className="space-y-3">
            ...
            {data.recent_uploads.map((upload) => (
              <div key={upload.upload_id} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
...
function Empty({ text }: { text: string }) {
  return <div className="rounded-2xl border border-dashed border-white/10 p-6 text-center text-sm text-text-muted">{text}</div>;
}
