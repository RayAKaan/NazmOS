...
def _days_since(value: Any) -> int | None:
    ...
    if isinstance(value, datetime):
        return max(0, (datetime.utcnow().date() - value.date()).days)
    if isinstance(value, date):
        return max(0, (datetime.utcnow().date() - value).days)
    return None
...
async def compute_money_audit(db: AsyncSession, business_id: UUID | str) -> AuditComputation:
    ...
    result = await db.execute(
        text("""
            ...
            sales_all AS (
                SELECT item_id, MAX(transaction_at) AS last_sold_any
                FROM transactions
                WHERE business_id = :business_id
                GROUP BY item_id
            )
            SELECT i.id AS item_id,
                   ...
                   COALESCE(s.last_sold_at, sa.last_sold_any) AS last_sold_at
            FROM items i
            LEFT JOIN inventory inv ON inv.item_id = i.id AND inv.business_id = i.business_id
    ...
    for row in rows:
        ...
        last_sold_days = _days_since(row.last_sold_at)
        ...
        if stock > 0 and cost > 0 and (qty_30d <= 0 or (last_sold_days is not None and last_sold_days >= DEAD_STOCK_DAYS)):
            ...
            actions.append({
                ...
                "quantity": stock,
                "recommended_discount_pct": Decimal("20"),
                "reason": "dead_stock",
            })
            ...
            if days_supply > OVERSTOCK_DAYS:
                ...
                if surplus_value >= Decimal("500"):
                    actions.append({
                        ...
                        "expected_recovery_sar": (surplus_value * Decimal("0.30")).quantize(Decimal("0.01")),
                        "quantity": surplus_qty.quantize(Decimal("0.01")),
                        "recommended_discount_pct": Decimal("15"),
                        "reason": "overstock",
                    })
        ...
        if daily_velocity > 0 and sell > 0:
            ...
            if days_left < STOCKOUT_DAYS:
                ...
                reorder_qty = max(Decimal("1"), (daily_velocity * MIN_REORDER_DAYS) - stock).quantize(Decimal("1"))
                actions.append({
                    "item_id": str(row.item_id),
                    "action_type": "reorder",
                    ...
                    "expected_recovery_sar": protected_sales,
            ...
            if margin_pct < TARGET_MARGIN_PCT:
                ...
                if leakage > 0:
                    ...
                    actions.append({
                        ...
                        "expected_recovery_sar": leakage,
                        "quantity": qty_30d,
                        "recommended_discount_pct": None,
                        "reason": "margin_leakage",
                    })

    actions = sorted(actions, key=lambda a: (a["priority"], -_float(a["expected_recovery_sar"])))[:MAX_ACTIONS]
    money_at_risk = (dead_stock_value + stockout_value + margin_leakage).quantize(Decimal("0.01"))
    confidence = Decimal(str(quality.get("score", 0))).quantize(Decimal("0.01"))
    ...
    if not rows:
        actions.append({
            ...
            "action_type": "review",
            "priority": 1,
            "title": "Upload sales and inventory files",
            "description": "NazmOS needs at least product names and current stock or sales history to generate a useful Money Audit.",
            "expected_recovery_sar": Decimal("0"),
            "quantity": None,
            "recommended_discount_pct": None,
            "reason": "no_data",
        })
    ...
    summary = {
        ...
        "money_at_risk_sar": _float(money_at_risk),
        "dead_stock_value_sar": _float(dead_stock_value),
        "stockout_risk_value_sar": _float(stockout_value),
        "margin_leakage_sar": _float(margin_leakage),
        "overstock_value_sar": _float(overstock_value),
        "money_approved_sar": 0.0,
        "money_recovered_sar": 0.0,
        "confidence_score": _float(confidence),
    ...
    for action in computation.actions:
        await db.execute(
            ...
            """),
            {
                ...
                "expected_recovery_sar": _float(action.get("expected_recovery_sar")),
                ...
                "evidence": json.dumps({"reason": action.get("reason")}, default=_json_default),
            },
        )
...
async def list_money_audit_actions(db: AsyncSession, audit_id: UUID | str) -> list[dict[str, Any]]:
    ...
    for row in result.fetchall():
        ...
        actions.append({
            ...
            "expected_recovery_sar": _float(row.expected_recovery_sar),
...
async def update_action_status(
    ...
    result = await db.execute(
        text("SELECT id, audit_id FROM money_audit_actions WHERE id = :id AND business_id = :business_id"),
...
def whatsapp_summary(audit: dict[str, Any]) -> str:
    ...
    lines = [
        "NazmOS Free Money Audit",
        f"Money at Risk: SAR {audit.get('money_at_risk_sar', 0):,.0f}",
        f"Dead stock: SAR {audit.get('dead_stock_value_sar', 0):,.0f}",
        f"Stockout risk: SAR {audit.get('stockout_risk_value_sar', 0):,.0f}",
        f"Margin leakage: SAR {audit.get('margin_leakage_sar', 0):,.0f}",
        "",
        "Top actions:",
    ]
    for idx, action in enumerate(top_actions, start=1):
        lines.append(f"{idx}. {action['title']} — expected SAR {action.get('expected_recovery_sar', 0):,.0f}")
    if not top_actions:
...
def printable_html(audit: dict[str, Any]) -> str:
    actions = "".join(
        ...
        <tr>
          ...
          <td>SAR {action.get('expected_recovery_sar', 0):,.0f}</td>
...
<div class='kpis'>
<div class='kpi'><span>Money at Risk</span><b class='red'>SAR {audit.get('money_at_risk_sar', 0):,.0f}</b></div>
