"""OpenCode Master System Prompt — the stable, never-generated runtime prompt.

This is PROMPT 2 of the NazmOS security architecture: the full 22-section
"isolated reasoning engine" system prompt that is delivered to OpenCode (as a
genuine system role) whenever NazmOS invokes it for reasoning.

Design rules for this module:
  - The prompt is a static constant. It is NEVER generated from merchant data.
  - It must remain DLP-clean (see app.security.dlp.DLP_RULES): it must not
    contain secret-like tokens, DB URLs, `key = value` secret assignments,
    long base64 runs, bearer tokens, JWTs, emails, KSA phones, or SQL/DDL
    keywords. It is scanned by _guard_outbound on every transport call.
  - Field names in the OUTPUT_CONTRACT must match what the output gate
    actually enforces (app.security.output_gate / ai_response_validator):
    decision, confidence, reasoning, evidence_ids, risk_flags,
    alternative_decision, challenge. (The draft prompt used "evidence_refs";
    we normalize to "evidence_ids".)
"""

MASTER_SYSTEM_PROMPT = """You are NazmOS's isolated reasoning engine.
...
# The exact JSON output schema appended to the system prompt. Kept in sync with
# app.security.output_gate.validate_ai_output and ai_response_validator.
OUTPUT_SCHEMA = """Return ONLY a single JSON object with this exact schema:
...
# Full prompt delivered to the model (system role body + output contract).
FULL_SYSTEM_PROMPT = MASTER_SYSTEM_PROMPT + "\n\n" + OUTPUT_SCHEMA

# Permission block used to lock down the agent to pure reasoning (no tools).
# These map to opencode agent permission keys (see opencode docs / agents).
...
def render_agent_markdown(
    agent_name: str = "nazmos-brain",
    description: str = "NazmOS isolated reasoning engine (system role)",
    mode: str = "primary",
) -> str:
    """Render a complete opencode agent .md file whose body IS the master
    system prompt and whose permissions are fully denied (pure reasoning)."""
    lines = ["---"]
    ...
    lines.append("---")
    lines.append("")
    lines.append(FULL_SYSTEM_PROMPT)
    return "\n".join(lines)
