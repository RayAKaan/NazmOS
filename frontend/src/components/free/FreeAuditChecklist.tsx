...
export function FreeAuditChecklist() {
  return (
    <div className="overflow-hidden rounded-3xl border border-brand-cream/10 bg-brand-night text-brand-cream shadow-2xl shadow-brand-night/20">
        ...
        <div className="p-6 md:p-7">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-brand-cream/62">
            ...
            <div className="inline-flex shrink-0 items-center gap-2 rounded-full bg-success/10 px-3 py-2 text-xs font-bold text-success">
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-4">
            {steps.map(([Icon, title, body], i) => (
              <div key={title} className="rounded-2xl bg-brand-cream/[0.04] p-4 ring-1 ring-brand-cream/5">
                <Icon className="mb-3 h-5 w-5 text-brand-green" />
                <p className="font-bold">{i + 1}. {title}</p>
                <p className="mt-1 text-sm leading-5 text-brand-cream/55">{body}</p>
              </div>
            ))}
          </div>

          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <Link href="/upload" className="inline-flex justify-center rounded-xl bg-brand-amber px-5 py-3 font-bold text-brand-night hover:bg-brand-gold">
              Start Free Audit
            </Link>
            <Link href="/product-demo" className="inline-flex justify-center rounded-xl border border-brand-cream/10 px-5 py-3 font-bold text-brand-cream/75 hover:bg-brand-cream/5 hover:text-brand-cream">
              See sample journey
            </Link>
          </div>
        </div>

        <div className="border-t border-brand-cream/10 bg-brand-cream/[0.03] p-6 md:p-7 lg:border-l lg:border-t-0">
          <div className="rounded-2xl border border-brand-amber/20 bg-brand-amber/10 p-4">
            ...
            <div className="mt-4 space-y-3">
              {fileColumns.map(([title, body]) => (
                <div key={title} className="rounded-xl bg-brand-night/20 p-3">
                  <p className="text-sm font-bold text-brand-cream">{title}</p>
                  <p className="mt-1 text-xs leading-5 text-brand-cream/55">{body}</p>
                </div>
              ))}
            </div>
          </div>

          <div className="mt-4 rounded-2xl border border-brand-cream/10 bg-brand-night/20 p-4">
            <div className="flex items-start gap-3">
              <LockKeyhole className="mt-0.5 h-5 w-5 text-brand-green" />
              <div>
                <p className="font-bold">User-first promise</p>
                <p className="mt-1 text-sm leading-6 text-brand-cream/55">
