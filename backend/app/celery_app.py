...
if not settings.USE_CELERY:
    ...
    def _health_ping() -> str:
        return "pong"

    def check_celery_health(timeout: float = 5.0) -> bool:
        """Ping every configured queue and return True only if all answer.

        The dead_letter queue has no dedicated consumer by design, so a missing
        reply there is expected and does not fail the check.
        """
        from app.utils.logger import setup_logger
        ...
        for queue in queues:
            try:
                result = celery_app.send_task("health.ping", queue=queue, args=[])
                if result.get(timeout=timeout) != "pong":
                    logger.warning("Health ping on queue returned unexpected reply", extra={"queue": queue})
                    all_healthy = False
            except Exception as exc:
                if queue == "dead_letter":
                    continue
                logger.warning("Health ping on queue failed", extra={"queue": queue, "error": str(exc)})
                all_healthy = False
        return all_healthy
